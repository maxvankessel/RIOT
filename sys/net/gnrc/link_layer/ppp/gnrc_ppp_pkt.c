...
static void _send_ppp_pkt(netdev_t *dev, gnrc_nettype_t protocol, uint8_t code, uint8_t id, gnrc_pktsnip_t *payload)
{
    gnrc_pktsnip_t *pkt = pkt_build(protocol, code, id, payload);

    send_packet((netdev_ppp_t *)dev, pkt);
}
void send_configure_request(netdev_t *dev, gnrc_nettype_t protocol, uint8_t id, gnrc_pktsnip_t *opts)
{
